...
const labels = {
  ...
  engagement: ['Digital experience', 'Initial window', 'Growing data history'],
} as const;
  ...
  return (
    <div className={`data-visual data-visual--${variant} ${large ? 'data-visual--large' : ''}`} aria-hidden="true">
      ...
      <div className="visual-grid">
        <div className="visual-metric"><small>{labels[variant][1]}</small><strong>{variant === 'archive' ? '10 yr' : variant === 'occupancy' ? '34 sites' : '12 mo'}</strong><span>{variant === 'engagement' ? 'Early-stage report' : 'Reporting scope'}</span></div>
