...
export function FeaturedReports() {
  return <div className="featured-reports">
    {featuredReports.map((report, index) => <article className={`featured-report featured-report-${index + 1}`} key={report.slug}>
      ...
      <h3>{report.title}</h3>
      <Image className="featured-report-preview" src={publicPath(`/images/reports/${report.preview}`)} alt={report.previewAlt} width={2213} height={1245} sizes="(max-width: 820px) 100vw, 50vw" draggable={false} />
