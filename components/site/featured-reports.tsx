...
export function FeaturedReports() {
  return <div className="featured-reports">
    {featuredReports.map((report, index) => <article className={`featured-report featured-report-${index + 1}`} key={report.slug}>
      ...
      <h3>{report.title}</h3>
      <Image className="featured-report-preview" src={publicPath(`/images/reports/${index === 0 ? 'quantara' : 'asterworks'}-first-page.png`)} alt={`${report.brand}: ${index === 0 ? 'Workplace Financial Performance' : 'Executive Performance'} report preview`} width={1174} height={660} sizes="(max-width: 820px) 100vw, 50vw" draggable={false} />
